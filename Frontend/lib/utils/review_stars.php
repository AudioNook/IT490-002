<?php 
// this shit took me a minute to figure out and it's probably not even an ideal way to do it
$averageRating = 3.5;

function renderStar($index, $averageRating) {
    if ($averageRating >= $index + 1) {
        return getFullStar();
    } elseif ($averageRating >= $index + 0.5) {
        return getHalfStar();
    } else {
        return getEmptyStar();
    }
